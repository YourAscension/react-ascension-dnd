...
function App() {
  ...
  const swapItemsInArray = (
    draggingElementId: number,
    elementToSwapId: number,
    items: IItems[]
  ) => {
    let itemsCopy = [...items];
    const draggingItem = itemsCopy.find(
      (item) => item.id === draggingElementId
    );

    itemsCopy = itemsCopy.filter((item) => item.id !== draggingElementId);

    const elementToSwapIndex = [...items].indexOf(
      [...items].find((item) => item.id === elementToSwapId)!
    );
  ...
  const swapItemsHandler = (
    ...
    setItems((prev) =>
      swapItemsInArray(draggingElementId, elementToSwapId, prev)
    );
  };
  ...
  return (
    <>
      ...
      <section className="section-two">
        <button onClick={() => setIsColumn(!isColumn)}>
          Set direction: {isColumn ? "column" : "row"}
        </button>
        <button onClick={addNewItemHandler}>Add new item</button>
        <DragAndDrop onSwapElement={swapItemsHandler}>
          {(dropZoneRef) => {
            return (
              <div
                ref={dropZoneRef}
              ...
              >
                {items.map((item) => {
                  return (
                    <DragAndDrop.Draggable key={item.id} id={item.id}>
                      {(draggableRef) => (
