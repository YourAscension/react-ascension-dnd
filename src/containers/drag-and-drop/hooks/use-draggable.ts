...
const useDraggable: UseDraggableType = (setIsDragging, dropZoneRef, draggableRef) => {
    return (event) => {
        const {clientX, clientY, pageX, pageY} = event
        if (dropZoneRef === undefined || draggableRef === undefined || dropZoneRef?.current === null || draggableRef?.current === null || setIsDragging === undefined) {
            return;
        }
